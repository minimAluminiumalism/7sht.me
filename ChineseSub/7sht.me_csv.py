...
def main():
	...
	for i in range(int(startpage), int(lastpage)+1):
		...
		for url in urls:
			...
			with open("7shtChineseSub.csv", "a") as csvfile:
				writer = csv.writer(csvfile)
				writer.writerow([data["Title"], data["UUID"], data["Actress"], data["Size"], data["Censored or not"], data["Magnet"]])
       
if __name__ == '__main__':
	with open("7shtChineseSub.csv", "w") as csvfile:
		writer = csv.writer(csvfile)
